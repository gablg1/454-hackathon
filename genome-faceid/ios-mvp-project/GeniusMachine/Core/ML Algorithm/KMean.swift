...
func getKMeanVectorSameName(vectors: [Vector], completionHandler: @escaping ([Vector]) -> Void) {
    Staff.shared.KMeans.reset()
    
    for i in 0..<vectors.count {
        Staff.shared.KMeans.addVector(vectors[i].vector)
    }
    
    Staff.shared.KMeans.clusteringNumber = AppConstants.NUMBER_OF_K

    Staff.shared.KMeans.clustering(500) { (success, centroids, clusters) -> () in
        if success {
            var vectorList:[Vector] = []
            for i in 0...2 {
                let vector = Vector(uuid: vectors[0].uuid, name: vectors[0].name, vector: centroids[i], distance: 0)
