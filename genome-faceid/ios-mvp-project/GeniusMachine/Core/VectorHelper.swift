...
class VectorHelper  {
    ...
    func createVector(name: String, uuid: String, image: UIImage) -> Vector? {
        //let frame = CIImage(image: image)!
        let frame = CIImage(image: image)!
        let img = Staff.shared.fDetector.extractFaces(frame: frame)
        if let i = img.first {
            let vector = Staff.shared.fnet.run(image: i)
            if vector.count  ==  128 {
                return (Vector(uuid: uuid, name: name, vector: vector))
            }
            else  { return nil }
        }
        else {
    ...
    func addVector(name: String, uuid: String, completionHandler: @escaping ([Vector]) -> Void) {
        let imageList = Staff.shared.trainingDataset.getImage(label: name)
        //print(imageList.count)
        var vectors: [Vector] = []
        print(imageList.count)
        if imageList.count > 0  {
            for item in imageList {
                if let vector = createVector(name: name, uuid: uuid, image: item!) {
    ...
    func loadVector() -> [Vector] {
        var vectors:[Vector] = []
        let items = Staff.shared.realm.objects(SavedVector.self)
        for item in items {
            let vector = Vector(uuid: item.uuid, name: item.name, vector: item.vector.map { $0 }, distance: item.distance)
            vectors.append(vector)
        }
        return vectors
    }
    ...
    func getResult(image: UIImage) -> Vector {
        var array: [Vector] = []
        var result = Vector(uuid: UUID().uuidString, name: "Unknown", vector: [], distance: 10)
        let image = image
        let frame = CIImage(image: image)!
        let img = Staff.shared.fDetector.extractFaces(frame: frame)
        if let i = img.first {
            let targetVector = Staff.shared.fnet.run(image: i)
    ...
    func getResult1(image: CIImage) -> Vector {
        var array: [Vector] = []
        var result = Vector(uuid: UUID().uuidString, name: "Unknown", vector: [], distance: 100)
//        guard let i = image.ciImage else { print("nil")
//            return result }
        let targetVector = Staff.shared.fnet.run(image: image)
